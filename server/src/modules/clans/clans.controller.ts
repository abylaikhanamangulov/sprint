...
const handleError = (res: Response, error: unknown) => {
  if ((error as Error).message === 'CLAN_NOT_FOUND') return res.status(404).json({ error: MESSAGES.errors.clanNotFound });
  if ((error as Error).message === 'USER_NOT_FOUND') return res.status(404).json({ error: MESSAGES.errors.userNotFound });
  if ((error as Error).message === 'NOT_ENOUGH_COINS') return res.status(400).json({ error: MESSAGES.errors.notEnoughCoins });
  if ((error as Error).message === 'ALREADY_IN_CLAN') return res.status(400).json({ error: MESSAGES.errors.alreadyInClan });
